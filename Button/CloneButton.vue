<template>
    ...
    >
        <span>{{ title ?? useTranslate("commons", "clone") }}</span>
        <template #content>
            <button
                v-bind="$attrs"
                :dusk="dusk"
                ...
                "
            >
                <material-icon :color="color"> content_copy</material-icon>
            </button>
        </template>
    </tool-tip>
</template>
...
defineProps({
    ...
    canClone: { type: Boolean, default: false, required: true },
    dusk: { type: String, default: "clone-button", required: false },
