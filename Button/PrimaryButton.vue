<template>
    <button
        ...
        v-bind="$attrs"
        class="transition-all duration-150 min-h-[2rem] select-none font-semibold py-2 px-4 focus:outline-hidden focus:shadow-outline flex items-center justify-center relative"
    >
...
const props = defineProps({
    ...
    textSize: { type: String, default: "text-xs", required: false },
    roundedStyle: { type: String, default: "rounded-sm", required: false },
    textStyle: { type: String, default: "uppercase", required: false },
...
watchEffect(() => {
    ...
    if (props.color === "white") {
        if (props.loading) {
            ...
            colorClass.value =
                props.variant === "outlined"
                    ? "ring-1 ring-gray-400 bg-white outline-hidden text-gray-700 hover:bg-gray-50  disabled:bg-gray-50 disabled:text-gray-300"
                    : "bg-white outline-hidden text-gray-700 hover:bg-gray-50  disabled:bg-gray-200 disabled:text-gray-300";
        }
    }
    ...
    if (props.color === "gray") {
        if (props.loading) {
            colorClass.value =
                props.variant === "outlined"
                    ? "bg-gray-200 text-gray-600 disabled:ring-gray-300 disabled:text-gray-300 ring-1 ring-gray-500 "
                    : "bg-white text-gray-600 disabled:ring-gray-300 disabled:text-gray-300";
        } else {
            colorClass.value =
                props.variant === "outlined"
                    ? "ring-1 ring-gray-300 bg-white outline-hidden text-gray-600 hover:bg-gray-50  disabled:bg-gray-200 disabled:text-gray-300"
                    : "bg-gray-200 outline-hidden text-gray-700 hover:bg-gray-300  disabled:bg-gray-200 disabled:text-gray-300";
        }
    }
});
</script>
